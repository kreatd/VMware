...
$vmnames = get-content movevms.txt
$vms = get-vm $vmnames -server $vcenter
$VmsToMove = $vms #| select -first 1

$cluster = get-cluster C16 -server $vcenter
...
foreach ($vm in $VmsToMove)
{
	...
	$DataDatastores = Get-DatastoreCluster -server $vcenter "dcscluster1" |get-datastore|sort freespacegb -Descending|select -first 5
	$OSDatastore = Get-DatastoreCluster -server $vcenter "dsccluster2" |get-datastore|sort freespacegb -Descending|select -Index ((get-random)%5)
